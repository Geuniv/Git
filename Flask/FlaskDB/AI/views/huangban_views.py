...
bp = Blueprint('huangban' , __name__ , url_prefix='/test')

@bp.route('/huangban')
def camera():
    return render_template('test/huangban.html')
...
disease = cv2.resize(cv2.imread('C:/Users/user/Desktop/pythonProject/pythonProject/Git/Flask/FlaskDB/AI/static/image/huangban\huangban.jpg'), (400, 400))
...
@bp.route('/huagban_camera')
def video_feed():
    global cap
    if Response(gen(cap),mimetype='multipart/x-mixed-replace; boundary=frame'):
        return render_template("test/huangban.html")    # 윈도우창이 출력시 카메라 페이지로 다시 돌아간다