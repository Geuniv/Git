from flask import render_template , Response , request , Blueprint , redirect
...
bp = Blueprint('astigmatism' , __name__ , url_prefix='/test')

@bp.route('/astigmatism')
@login_required # @login_required 데코레이터
def astigmatism():
    return render_template('test/astigmatism.html')
...
disease = cv2.resize(cv2.imread('C:/Users/user/Desktop/pythonProject/pythonProject/Git/Flask/FlaskDB/AI/static/image/astigmatism/pikacyu.jpg'),(640,430))
...
@bp.route('/astigmatism_camera')
def video_feed():
    global cap
    if Response(gen(cap),mimetype='multipart/x-mixed-replace; boundary=frame'):
        return render_template("test/astigmatism.html")    # 윈도우창이 출력시 카메라 페이지로 다시 돌아간다