...
class UserCreateForm(FlaskForm):
    username = StringField('사용자이름', validators=[DataRequired("ID를 확인해주세요"), Length(min=3, max=25)]) # Length 오류 메세지 찾는중 + 이메일
    ...
    name = StringField('이름' , validators=[DataRequired("이름을 확인해주세요")])
    ...
    address1 = StringField('주소' , validators=[DataRequired("주소를 확인해주세요") , Length(max=100)])
    address2 = StringField('상세주소' , validators=[DataRequired("상세주소를 확인해주세요") , Length(max=100)])
    email = EmailField('이메일', validators=[DataRequired("이메일을 확인해주세요"), Email()])
    phone = StringField('전화번호', validators=[DataRequired("전화번호를 확인해주세요") , Length(max=13)])
...
class CataractForm(FlaskForm):
    cataract = StringField('발병여부' , validators=[DataRequired()])
    accuracy = IntegerField('정확도' , validators=[DataRequired()])