def df_Search_NaverKin(keyword):
    import json
    ...
    # 빈 데이터프레임 생성
    df_NaverKin = pd.DataFrame(columns=['Title', 'Link', 'Description'])

    for i in range(start, end, display):
        # JSON url
        url_query = "https://openapi.naver.com/v1/search/kin?query=" + query
        ...
        if (rescode == 200):
            ...
            for item_idx in range(len(items)):
                ...
                df_NaverKin.loc[idx] = [title, link, description]
                idx += 1
        else:
